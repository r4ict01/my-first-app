...
function draw() {
    ...
    ctx.fillStyle = '#171a36'; for (let i = 0; i < 24; i++) { const x = i * 210 - camera * .32; const height = 28 + (i % 4) * 16; const buildingWidth = 90 + (i % 3) * 20; ctx.fillRect(x, 350 - height, buildingWidth, height); ctx.save(); ctx.shadowColor = i % 3 === 0 ? '#ff2079' : '#73e1dc'; ctx.shadowBlur = 7; ctx.fillStyle = i % 3 === 0 ? '#ff7aa9' : '#a5ffff'; ctx.globalAlpha = .7; for (let y = 350 - height + 10; y < 342; y += 13) { ctx.fillRect(x + 10, y, 5, 4); if (buildingWidth > 100) ctx.fillRect(x + 32, y, 5, 4); if (i % 4 === 0) ctx.fillRect(x + 54, y, 5, 4); } ctx.restore(); ctx.globalAlpha = 1; }
